...
export function useShoppingTypedDraft({ userId, projectId, enabled, active: isActive = true }) {
  ...
  const scope = useMemo(() => ({ userId, projectId, store, isActive }), [userId, projectId, store, isActive]);
  ...
  useEffect(() => {
    if (!isActive) { current.current = null; setValue(''); setError(''); return; }
    setError('');
    if (!enabled) {
  ...
  }, [enabled, isActive, legacyKey, projectId, store, userId]);

  const set = useCallback(next => {
    if (!isActive || active.current !== scope) return;
  ...
  }, [enabled, isActive, legacyKey, projectId, scope, store]);

  const prepare = useCallback(() => {
    if (!isActive || active.current !== scope || !current.current?.generation) throw new Error('Shopping draft is still opening.');
    // Retry exactly the in-memory generation after failed persistence.
    store.persistDraft(current.current);
    return current.current;
  }, [isActive, scope, store]);
  ...
  }, [isActive, projectId, scope, store]);
  const clearAccepted = useCallback(generation => {
    if (!isActive || active.current !== scope || current.current?.generation !== generation) return;
