  ...
  return (
    <div className="pm-shell-bg pm-accent-scope flex min-h-screen w-full flex-col overflow-x-hidden">
      <div className="border-b border-slate-200 bg-white/90 backdrop-blur">
        <div className={`mx-auto w-full max-w-7xl sm:px-6 sm:py-4 ${compactMobile ? 'px-3 py-2' : 'px-4 py-3'}`}>
          <div className={`flex gap-3 sm:flex-row sm:items-center sm:justify-between ${compactMobile ? 'items-center justify-between' : 'flex-col'}`}>
            <div className="min-w-0">
              <div className="flex items-center gap-3">
                <PmWorkspaceLogo iconOnly size="xs" />
                <div className="min-w-0">
                  <h1 className={`truncate font-bold text-slate-950 sm:text-lg ${compactMobile ? 'text-[13px]' : 'text-sm'}`}>{title}</h1>
                </div>
              </div>
              <p className="mt-1 hidden truncate text-[11px] text-slate-500 sm:block sm:text-sm">{userEmail}</p>
            </div>

            <div className={`flex max-w-full items-center gap-2 sm:w-auto sm:flex-wrap sm:justify-end ${compactMobile ? 'w-auto flex-nowrap' : 'w-full flex-wrap'}`}>
              <div className="hidden sm:block">
                <AccentThemePicker value={accentTheme} onChange={onAccentThemeChange} />
              </div>
              <button
                onClick={onGoToProjects}
                className={`rounded-xl border border-slate-200 bg-white font-semibold text-slate-600 transition hover:border-slate-300 hover:text-slate-900 sm:px-4 sm:py-2.5 sm:text-sm ${compactMobile ? 'min-h-[44px] px-2.5 py-2 text-[11px]' : 'px-3.5 py-2.5 text-xs'}`}
              >
                Projects
              </button>
              <button
